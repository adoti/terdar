...
class Trade:
	...
	def __init__(self, entry, target, exit, risk):
		self.entry = entry
		self.target = target
		self.exit = exit
		self.risk = risk
	...
	def update(self, entry, target, exit, risk):
		self.entry = entry
		self.target = target
		self.exit = exit
		self.risk = risk
	...
	def get_rr(self):
		return -1*self.target_return()/self.exit_return()
	...
	def target_return(self):
		if self.direction:
			if self.direction == "LONG":
				return self.get_position_size() * (self.target - self.entry - self.limit_fee(self.target) - self.market_fee(self.entry))
			else:
				return self.get_position_size() * (self.entry - self.target - self.limit_fee(self.target) - self.market_fee(self.entry))
		return None
	...
	def exit_return(self):
		if self.direction:
			if self.direction == "LONG":
				return self.get_position_size() * (self.exit - self.entry - self.limit_fee(self.exit) - self.market_fee(self.entry))
			else:
				return self.get_position_size() * (self.entry - self.exit - self.limit_fee(self.exit) - self.market_fee(self.entry))
		return None
